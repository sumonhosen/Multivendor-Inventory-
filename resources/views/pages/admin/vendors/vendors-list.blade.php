...
@section('title', 'Distributor List' .' < '. get_site_title())

@section('content')
<h4 class="box-title">Distributor List</h4><hr class="text-border-bottom">
          ...
          <tbody>
            @if(count($vendors_list_data)>0)
              @foreach($vendors_list_data as $row)
                ...
                <tr>
                  <td><a href="" data-email="{{ $row->email }}" class="specific-vendor-for-contact">{{ trans('admin.contact_label') }}</a></td>
...
<div class="modal fade" id="vendors_contact" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <p class="no-margin popup-title"></p>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
        </button>
      </div>
            ...
            <form method="post" action="" enctype="multipart/form-data">
              <div class="box-body">
                <div class="form-group">
                  <input type="text" class="form-control" name="vendormailsubject" id="vendormailsubject" placeholder="Subject">
                </div>
                <div>
                  <textarea id="vendormailbody" name="vendormailbody" placeholder="Message......." style="width: 100%; height: 125px;font-size: 14px;border: 1px solid #dddddd; padding: 10px;"></textarea>
                </div>
              </div>  
            </form>
          </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary send-email-to-vendor" data-admin_email="{{ $user_data['user_email'] }}" data-vendor_email="">{!! trans('admin.send') !!}</button>
        <button type="button" class="btn btn-primary" data-dismiss="modal">{!! trans('admin.close') !!}</button>
      </div>
    </div>
  </div>
