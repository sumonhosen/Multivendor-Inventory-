...
class PaymentMethodController extends Controller
{
  ...
  public function paymentMethodRazorpayContent(){
    $data = array();
    ...
    if(is_vendor_login()){
      return view('pages.admin.payment.vendor-payment-razorpay', $data);
    }
    else{
      return view('pages.admin.payment.payment-razorpay', $data);
    }
  }
  ...
  public function savePaymentData(){
    if( Request::isMethod('post') && Session::token() == Request::Input('_token') ){
      ...
      elseif (Request::Input('_payment_method_type') == 'razorpay') {
        $isRazorpayEnable        = 'no';
        $isRazorpaySandboxEnable = 'no';
        $enable_razorpay         = (Request::has('inputEnablePaymentRazorpayMethod')) ? true : false;
        $enable_sandbox        = (Request::has('inputEnableRazorpaySandboxOption')) ? true : false;
        
        if($enable_razorpay){
          $isRazorpayEnable = 'yes';
        }
        else{
          $isRazorpayEnable = 'no';
        }
        ...
        if(is_vendor_login() && !empty($vendor_data)){
          $vendor_data->payment_method->razorpay->status         =  $isRazorpayEnable;
          $vendor_data->payment_method->razorpay->title          =  Request::Input('inputRazorpayTitle');
          $vendor_data->payment_method->razorpay->email_id       =  Request::Input('inputRazorpayEmail');
          $vendor_data->payment_method->razorpay->description    =  Request::Input('inputRazorpayDescription');
          ...
          $get_return_payment_data = $this->create_payment_array_data('razorpay', array('status' => $isRazorpayEnable, 'title' => Request::Input('inputRazorpayTitle'), 'client_id' => Request::Input('inputRazorpayClientId'), 'secret' => Request::Input('inputRazorpaySecret'), 'sandbox_status' => $isRazorpaySandboxEnable,  'description' => Request::Input('inputRazorpayDescription')));
        }
      }
  ...
  public function create_payment_array_data($source, $array){
    ...
    $paypal_method_desc            =   '';
    $enable_razorpay               =   '';
    $razorpay_method_title         =   '';
    $razorpay_client_id            =   '';
    $razorpay_secret               =   '';
    $enable_razorpay_sandbox       =   '';
    $razorpay_method_desc          =   '';
    $enable_stripe                 =   '';
    $stripe_method_title           =   '';
    $test_secret_key               =   '';
    ...
    if($source == 'razorpay'){
      $enable_razorpay                =   $array['status'];
      $razorpay_method_title          =   $array['title'];
      $razorpay_client_id             =   $array['client_id'];
      $razorpay_secret                =   $array['secret'];
      $enable_razorpay_sandbox        =   $array['sandbox_status'];
      $razorpay_method_desc           =   $array['description'];
    }
    else{
      $enable_razorpay                =   $unserialize_data['razorpay']['enable_option'];
      $razorpay_method_title          =   $unserialize_data['razorpay']['method_title'];
      $razorpay_client_id             =   $unserialize_data['razorpay']['razorpay_client_id'];
      $razorpay_secret                =   $unserialize_data['razorpay']['razorpay_secret'];
      $enable_razorpay_sandbox        =   $unserialize_data['razorpay']['razorpay_sandbox_enable_option'];
      $razorpay_method_desc           =   $unserialize_data['razorpay']['method_description'];
    }
    ...
    $payment_method_array = array( 
      ...
      'razorpay'         => array('enable_option' => $enable_razorpay, 'method_title' => $razorpay_method_title, 'razorpay_client_id' => $razorpay_client_id, 'razorpay_secret' => $razorpay_secret, 'razorpay_sandbox_enable_option' => $enable_razorpay_sandbox, 'method_description' => $razorpay_method_desc),
