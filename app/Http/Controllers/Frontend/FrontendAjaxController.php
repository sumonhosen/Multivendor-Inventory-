...
class FrontendAjaxController extends Controller
{
  ...
  public function productAddToCart()
  {
    ...
    if(!empty($input['affiliation_id'])){
      if(Session::get('affiliation')){
        $affialiation = Session::get('affiliation');
      ...
      Session::put('affiliation',$affialiation);
    }
   ...
   * @return void
   */
  public function productAddToCartOffer()
  {
    if(Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')){
      $input = Request::all();
     
      if(isset($input['variation_id']) && $input['variation_id']){
        $this->classCommonFunction->add_to_cart_offer($input['product_id'],$input['price'], $input['offer_product_name'], $input['qty'], $input['variation_id'], $input['selected_option']);
      }
      else{
        $this->classCommonFunction->add_to_cart_offer($input['product_id'],$input['price'], $input['offer_product_name'], $input['qty']);
      }
    }
  }
  ...
  */
      ...
      if(isset($get_current_user_id['user_id']) && $get_current_user_id['user_id']){ 
        ...
        if(count($get_data_by_user_id) > 0){
          ...
          if(isset($parse_details['wishlists_details'])){
            ...
            $usr=DB::table('users_details')->where('user_id', $get_current_user_id['user_id'])->select('users_details.*')   ->first();  

            $udata=json_decode($usr->details,true);
            $udata1 = array('wishlists_details' => '');

            $result = array_merge($udata, $udata1);
            $messages = json_encode($result);

            DB::table('users_details')->where('user_id', $get_current_user_id['user_id'])->update( array('details' =>$messages , ));  
             if($this->classCommonFunction->frontendUserAccountDataProcess( $wishlist_data )){
                return response()->json(array('status' => 'success', 'notice_type' => 'user_wishlist_saved'));
              }
  ...
  public function getMiniCartData(){
    ...
    if(Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')){
      ...
      elseif($input['mini_cart_id'] == 2){
        $returnHTML = view('pages.ajax-pages.mini-cart-html-second')->render();
        //$n = fopen("m.txt","w");
    //   // fwrite($n,$returnHTML);
  ...
  public function contactWithSendEmail(){
    //if(Request::isMethod('post') && Session::token() == Request::header('X-CSRF-TOKEN')){
      ...
      $mailData = array();
      print_r($input);die;
      $mail_to = 'sumanpattanayak58@gmail.com';
