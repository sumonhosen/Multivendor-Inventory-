...
class GetFunction
{
  ...
  public function get_orders_by_date_range($start_date, $end_date)
  {
    ...
    $getOrdersByDate = $order->manageAllOrders( $getOrdersByDate );
    return  $getOrdersByDate;
  }
  
   /**
   * Get function for Search Result
   *
   * @param type,keyword
   * @return obj
   */
  public function search_orders_by_select($type=null,$keyword=null)
  {
    $order=new OrderController();

    if($type == "_billing_first_name" || $type == '_final_order_total'){
         $getOrdersByDate = DB::table('posts')
                      ...
                      ->where(function ($query) use ($type, $keyword) {
                        ...
                        if($type == "_billing_first_name"){
                            $query->whereRaw('post_extras.key_name = "_billing_first_name" AND post_extras.key_value like "%'.$keyword.'%" ');
                        }

                        if($type == "_final_order_total"){
                            $query->WhereRaw('post_extras.key_name = "_final_order_total" AND post_extras.key_value = '.$keyword.' ');
                        }
      ...
      $getOrdersByDate = DB::table('posts')
                         ->select(DB::raw('posts.*, users.id as uid'))
                         ->where(['posts.post_type' => 'shop_order']) 
                         ->join('post_extras','post_extras.post_id', '=', 'posts.id')
                       ...
                       ->where(function ($query) use ($type, $keyword) {
                         ...
                         if($type == "distributor"){
                              $query->WhereRaw('users.unique_id like "%'.$keyword.'%"  ');
     ...
     $getOrdersByDate = DB::table('posts')
                        ->select(DB::raw('posts.*, order_products.id as oid, products.id as pid'))
                        ->where(['posts.post_type' => 'shop_order']) 
                        ->join('post_extras','post_extras.post_id', '=', 'posts.id')

                        ->join("order_products",function($join){
                            $join->on("order_products.order_id","=","posts.id");
                        })
                      ...
                      ->where(function ($query) use ($type, $keyword) {
                        ...
                        if($type == "product"){
                             $query->WhereRaw('products.title like "%'.$keyword.'%"  ');
    ...
    //converting objects into array
           $getOrdersByDate->transform(function($i) {
              return (array)$i;
          });
          $getOrdersByDate = $getOrdersByDate->toArray();

    $getOrdersByDate = $order->manageAllOrders( $getOrdersByDate );

    // print_r($getOrdersByDate);
    return  $getOrdersByDate;
  }
   ...
   * Get function for Search Result
   *
   * @param Name,Price,Product Name,startdate,End Date
   * @return obj
   */
  public function search_orders($name=null,$price=null,$start_date=null,$end_date=null)
  {
    $order=new OrderController();
     ...
     $getOrdersByDate = DB::table('posts')
                      ...
                      ->where(function ($query) use ($name, $price,$start_date,$end_date) {

                        if($name){
                            $query->whereRaw('post_extras.key_name = "_billing_first_name" AND post_extras.key_value like "%'.$name.'%" ');
                        }

                        if($price){
                            $query->WhereRaw('post_extras.key_name = "_final_order_total" AND post_extras.key_value = '.$price.' ');
                        }

                        if($start_date && $end_date){
                           $query->whereBetween('posts.created_at', array($start_date, $end_date.' 23:59:59'));
                        }
      ...
      foreach($productsTitleOrders as $row){
        if(!empty($row)){
          ...
          foreach($parse_order_data as $count_row){
            if(!empty($count_row)){
              $index_name = string_slug_format($count_row->name).'_'. $count_row->id;

              if(!array_key_exists($index_name, $sale_by_product_title) ){
                $sale_by_product_title[$index_name] = array('id' => $count_row->id, 'product_title' => $count_row->name, 'units_sold' => $count_row->quantity, 'gross_sales' => $count_row->quantity * $count_row->price, 'currency' => get_currency_symbol_by_code(PostExtra::where(['post_id' => $row->id, 'key_name' => '_order_currency'])->first()->key_value), 'gross_sales_with_currency' => price_html( $count_row->quantity * $count_row->price ));
              }
              elseif (array_key_exists($index_name, $sale_by_product_title)) {
                $sale_by_product_title[$index_name]['units_sold']  +=  $count_row->quantity;
                $sale_by_product_title[$index_name]['gross_sales'] +=  $count_row->quantity * $count_row->price;
                $sale_by_product_title[$index_name]['gross_sales_with_currency'] =  price_html( $sale_by_product_title[$index_name]['gross_sales'] );
              }
            }
          }
        }
  ...
  public function get_order_by_specific_date_range($daysOrders)
  {
    ...
    {
      $order_data = array();
      $order_data['order_id']       = $rows['_post_id'];
      $order_data['order_date']     = $this->carbonObject->parse($rows['_order_date'])->toDayDateTimeString();
      $order_data['order_status']   = PostExtra::where(['post_id' => $rows['_post_id'], 'key_name' => '_order_status'])->first()->key_value;
      $order_data['order_totals']   = PostExtra::where(['post_id' => $rows['_post_id'], 'key_name' => '_order_total'])->first()->key_value;
      $order_data['order_totals_with_currency']   = price_html( PostExtra::where(['post_id' => $rows['_post_id'], 'key_name' => '_order_total'])->first()->key_value );
   ...
   * @return array
   */
  public function get_order_by_month($daysOrders)
  {
    $daysOrdersData = array();
   ...
   * @return array
   */
  public function get_reports_coupons_totals(){
    $coupons_totals = [
      [
        'handle' => 'discount_from_product',
        'name' => 'Discount from product',
        'totals' => PostExtra::where(['key_name' => '_coupon_condition_type', 'key_value' => 'discount_from_product'])->get()->count()
      ],
      [
        'handle' => 'percentage_discount_from_product',
        'name' => 'Percentage discount from product',
        'totals' => PostExtra::where(['key_name' => '_coupon_condition_type', 'key_value' => 'percentage_discount_from_product'])->get()->count()
      ],
      [
        'handle' => 'discount_from_total_cart',
        'name' => 'Discount from total cart',
        'totals' => PostExtra::where(['key_name' => '_coupon_condition_type', 'key_value' => 'discount_from_total_cart'])->get()->count()
      ],
      [
        'handle' => 'percentage_discount_from_total_cart',
        'name' => 'Percentage discount from total cart',
        'totals' => PostExtra::where(['key_name' => '_coupon_condition_type', 'key_value' => 'percentage_discount_from_total_cart'])->get()->count()
      ]
    ];

    return $coupons_totals;
  }
   ...
   * @return array
   */
  public function get_reports_role_based_user_totals(){
    $users = [
      'users_totals' => User::get()->count(),
      'user_roles_totals' => Role::get()->count(),
      'roles_based_users_totals' => DB::table('roles')
                                        ->join('role_user', 'role_user.role_id', '=', 'roles.id')
                                        ->select(DB::raw("roles.role_name as users_totals"), DB::raw("roles.slug as handle"), DB::raw("COUNT(role_user.user_id) as count"))
                                        ->groupBy(DB::raw("role_user.role_id"))
                                        ->get()
                                        ->toArray()
    ];
   ...
   * @return array
   */
  public function get_reports_orders_totals(){
    $total_orders = [
      'orders_totals' => Post::where(['post_type' => 'shop_order', 'parent_id' => 0])->get()->count(),
      'status_based_orders_totals' => [
        [
          'handle' => 'pending',
          'name' => 'Pending payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'pending'])->get()->count()
        ],
        [
          'handle' => 'processing',
          'name' => 'Processing payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'processing'])->get()->count()
        ],
        [
          'handle' => 'on-hold',
          'name' => 'On hold',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'on-hold'])->get()->count()
        ],
        [
          'handle' => 'completed',
          'name' => 'Completed payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'completed'])->get()->count()
        ],
        [
          'handle' => 'cancelled',
          'name' => 'Cancelled payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'cancelled'])->get()->count()
        ],
        [
          'handle' => 'refunded',
          'name' => 'Refunded payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'refunded'])->get()->count()
        ],
        [
          'handle' => 'shipping',
          'name' => 'Shipping payment',
          'totals' => PostExtra::where(['key_name' => '_order_status', 'key_value' => 'shipping'])->get()->count()
        ]
      ]
   ...
   * @return array
   */
  public function get_reports_products_totals(){
    $totals_products = [
      'products_totals' => Product::get()->count(),
      'type_based_products_totals' => [
        [
          'handle' => 'simple_product',
          'name' => 'Simple product',
          'totals' => Product::where(['type' => 'simple_product'])->get()->count()
        ],
        [
          'handle' => 'configurable_product',
          'name' => 'Configurable product',
          'totals' => Product::where(['type' => 'configurable_product'])->get()->count()
        ],
        [
          'handle' => 'customizable_product',
          'name' => 'Customizable product',
          'totals' => Product::where(['type' => 'customizable_product'])->get()->count()
        ],
        [
          'handle' => 'downloadable_product',
          'name' => 'Downloadable product',
          'totals' => Product::where(['type' => 'downloadable_product'])->get()->count()
        ]
      ]
    ];

    return $totals_products; 
  }
   ...
   * @return array
   */
  public function get_reports_todays_orders(){
   ...
   * @return array
   */
  public function get_reports_todays_orders_totals(){
    $todaysTotalSale = 0;
    $todaysTotalData = array();
    ...
    if(is_array($todayOrders) && count($todayOrders) > 0){
      foreach($todayOrders as $order){
        $get_order_total = PostExtra::where(['post_id' => $order['id'], 'key_name' => '_order_total'])->first();

        if(!empty($get_order_total->key_value)){
          $todaysTotalSale += $get_order_total->key_value;
        }
      }
    }   
    
    $todaysTotalData = [
      'todays_total_sale' => $todaysTotalSale,
      'todays_orders' => $todayOrders  
    ];

    return $todaysTotalData;                       
  }  
  ...
  {
    $payment = $this->option->getPaymentMethodData();

    $mode = 'sandbox';
    if($payment['paypal']['paypal_sandbox_enable_option'] == 'no')
    {
      $mode = 'live';
    }
    
    return array('client_id' => $payment['paypal']['paypal_client_id'], 'secret' => $payment['paypal']['paypal_secret'], 'settings' => array('mode' => $mode, 'http.ConnectionTimeOut' => 30, 'log.LogEnabled' => true, 'log.FileName' => storage_path() . '/logs/paypal.log', 'log.LogLevel' => 'FINE'));
  }
  ...
  public function sendCustomMail($data){
    ...
    $get_view_data =  array();
    $site_title    =  'Ceyone';
    $email         =  'sumanpattanayak58@gmail.com';
    $logo          =  default_placeholder_img_src();
    $appearance    =  array();
    
    $email_options = get_emails_option_data();
    $settingsData = $this->option->getSettingsData();
      
    if(isset($settingsData['general_settings']['general_options']['site_title']) && $settingsData['general_settings']['general_options']['site_title']){
      $site_title = $settingsData['general_settings']['general_options']['site_title'];
    }

    if(isset($settingsData['general_settings']['general_options']['email_address']) && $settingsData['general_settings']['general_options']['email_address']){
      $email = $settingsData['general_settings']['general_options']['email_address'];
    }
   //echo  $email;die;
    $email         =  'sumanpattanayak58@gmail.com';
    if(isset($settingsData['general_settings']['general_options']['site_logo']) && $settingsData['general_settings']['general_options']['site_logo']){
      $logo = $settingsData['general_settings']['general_options']['site_logo'];
    }
    ...
    if($data['source'] == 'order_confirmation'){ 
      ...
      if($get_post_meta && $get_post_meta->count() > 0){
        ...
        $get_view_data['_payment_method_details'] =  '';
      // $get_view_data['_payment_method_details'] =  $payment[$get_view_data['_payment_method']];
    ...
    elseif($data['source'] == 'new_customer_account'){
      ...
      $get_view_data['_view']        =   $view;
      $get_view_data['name']        =   $data['dis_name'];
      $get_view_data['_mail_to']    =   $data['email'];
      $get_view_data['password']    =   $data['password'];
    ...
    elseif($data['source'] == 'vendor_account_activation'){
      ...
      $get_view_data['_view']       =   $view;
      $get_view_data['_mail_to']    =   $data['email'];
      $get_view_data['name']        =   $data['name'];
      $get_view_data['unique_id']   =   $data['unique_id'];
  ...
  public static function variation_product_price($product_id){
    $final_price  =  0;
    $get_product  =  PostExtra :: where(['post_id' => $product_id, 'key_name' => '_variation_post_price'])->first();
    
    if(!empty($get_product)){
      $final_price  = $get_product->key_value;
    }
  ...
  public static function get_country_name_by_code( $code ){
    ...
    if( !in_array($country_code, $countryList)) {
      return $country_code;
    }
    else {
  ...
  {
    $_this = new self;
    $payment = $_this->option->getPaymentMethodData();
    
    if(isset($payment[$payment_method]['method_title']))
    {
      return $payment[$payment_method]['method_title'];
    }
  }
  ...
  public static function current_admin_user_info(){
    ...
    if(Session::has('shopist_admin_user_id')){
      $getuserdata = DB::table('users')
                         ->where('users.id', Session::get('shopist_admin_user_id'))
                         ->join('role_user', 'users.id', '=', 'role_user.user_id')
                         ->join('roles', 'role_user.role_id', '=', 'roles.id')
                         ->select('users.*', DB::raw('roles.id as role_id'), 'roles.role_name', 'roles.slug')
                         ->first();
 
      if(!empty($getuserdata)){
        $userData['user_role_id'] = $getuserdata->role_id;
        $userData['user_role'] = $getuserdata->role_name;
        $userData['user_role_slug'] = $getuserdata->slug;
  ...
  public static function current_frontend_user_info(){
    ...
    if(Session::has('shopist_frontend_user_id')){
      ...
      if(!empty($getuserdata)){
        ...
        $userData['member_since'] = $getuserdata->created_at;
        $userData['bv_points']    = $getuserdata->bv_points;
        $userData['affiliation_points']    = $getuserdata->appliation_point;
        $userData['appliation_id']    = $getuserdata->appliation_id;
        $userData['unique_id']    = $getuserdata->unique_id;
      }
    }
    else{
      $userData['user_role_id'] = '';
  ...
  public static function create_price_view_for_list($price, $getSettings){
    $_this      =   new self;
    $currency   =   '';
    $price_html =   '';
  ...
  public static function get_user_all_details( $user_id ){
    ...
    if($getuserdata && count($getuserdata->roles) > 0){
      $userData['user_role']          =   $getuserdata->roles[0]->role_name;
      $userData['role_id']            =   $getuserdata->roles[0]->id;
      $userData['user_role_slug']     =   $getuserdata->roles[0]->slug;
      $userData['user_display_name']  =   $getuserdata->display_name;
      $userData['user_name']          =   $getuserdata->name;
      $userData['user_email']         =   $getuserdata->email;
  ...
  public static function subscription_data(){
    ...
    $option = $_this->option->getSubscriptionData();
    
    if(count($option) > 0){
      $subscription_data_array = $option; 
    }
  ...
  public static function check_sufficient_permission(){
    ...
    if(count($currentAdminUserData) > 0){
      ...
      elseif((Request::is('admin/rest-api-token') || Request::is('admin/rest-api-token/*')) && $get_role_data->slug != 'administrator'){
        return false;
      }
      else{
        return true;
  ...
  public static function stripe_api_key(){
    $_this  =  new self;
    $payment_method = $_this->option->getPaymentMethodData();
  ...
  public static function razorpay_api_key(){
    $_this  =  new self;
    $payment_method = $_this->option->getPaymentMethodData();
    $razorpay_api_key = array('secret_key' => '', 'publishable_key' => '');
    
    if(count($payment_method) > 0 && isset($payment_method['razorpay'])){
      if($payment_method['razorpay']['razorpay_sandbox_enable_option'] == 'yes'){
        $razorpay_api_key['secret_key'] =  $payment_method['razorpay']['razorpay_secret'];
        $razorpay_api_key['publishable_key'] =  $payment_method['razorpay']['razorpay_client_id'];
      }
      else{
        $razorpay_api_key['secret_key'] =  $payment_method['razorpay']['razorpay_secret'];
        $razorpay_api_key['publishable_key'] =  $payment_method['razorpay']['razorpay_client_id'];
  ...
  public static function recaptcha_data(){
    ...
    $_this  =  new self;
    $settings_data = $_this->option->getSettingsData();
  ...
  public static function vendor_name_by_oid($order_id){
    $vendor_name = '';
    $get_author_id  = Post::where(['id' => $order_id])->first();
    
    if(!empty($get_author_id)){
          $vendor_details = User::where(['id' => $get_author_id->post_author_id])->first();

        if(isset($vendor_details) && $vendor_details->unique_id != '' ){
            $vendor_name = '<i>'. $vendor_details->display_name .' ('. $vendor_details->unique_id .')</i>';
          } else {
            $vendor_name='';
          }
  ...
  public static function permission_menu_heading($heading_slug, $user_permission_list){
    ...
    // }
    if(!is_array($user_permission_list) && count($user_permission_list) == 0){ return false;}  
  ...
  public static function generate_unique_slug($table, $input_field){
    $slug = '';
    $field = '';
    $obj = null;
    ...
    if($table == 'post'){
      $post = new Post;
      $get_obj = Post::where(['post_slug' => string_slug_format( $input_field )])->orWhere('post_slug', 'like', '%' . string_slug_format( $input_field ) . '%')->first();
      
      if(!empty($get_obj)){
        $field = $get_obj->post_slug;
      }
      
      $obj = $get_obj;
    }
    else if($table == 'post_extras'){
      $post = new PostExtra;
      $get_obj = PostExtra::where(['key_value' => string_slug_format( $input_field )])->orWhere('key_value', 'like', '%' . string_slug_format( $input_field ) . '%')->first();

      if(!empty($get_obj)){
        $field = $get_obj->key_value;
      }
      ...
      $post = new Term;
      $get_obj = Term::where(['slug' => string_slug_format( $input_field )])->orWhere('slug', 'like', '%' . string_slug_format( $input_field ) . '%')->first();
      ...
      $post = new Product;
      $get_obj = Product::where(['slug' => string_slug_format( $input_field )])->orWhere('slug', 'like', '%' . string_slug_format( $input_field ) . '%')->first();
    ...
    if(!empty($obj)){
      if($obj->count() > 1){
        $count = $obj->count();
        $slug = string_slug_format( $input_field ). '-' . $count;
      }
      else{
        $slug = string_slug_format( $input_field ). '-' . 1;
      }
    }
    else{
      $slug = string_slug_format( $input_field );
    }
    
    return $slug;
  }
  
  public static function rest_api_data_by_token($token){
      $data = array();
      $get_data = ApiToken::where('token', $token)->first();
      
      if(!empty($get_data)){
          $data['id'] = $get_data->id;
          $data['title'] = $get_data->title;
          $data['user_id'] = $get_data->user_id;
          $data['permissions'] = $get_data->permissions;
          $data['token'] = $get_data->token;
          $data['created_at'] = $get_data->created_at;
          $data['updated_at'] = $get_data->updated_at;
          $data['user_details'] = get_user_details($get_data->user_id);
      }
      
      return $data;
  }

  public static function check_is_installed(){
    static $is = null;

    if ($is === null) {
      $is = Config::get('shopist.is_installed');
    }

    return (bool) $is;
  }
  
  public static function check_has_admin(){
    static $is = null;

    if ($is === null) {
      $is = Config::get('shopist.has_admin');
    }

    return (bool) $is;
  }
  
  public static function get_install_config(){
    return Config::get('shopist');
  }

  public static function is_all_tables_created_in_db(){
    $is_all_tables_created = false;

    if(\Schema::hasTable('api_tokens') || \Schema::hasTable('comments') || \Schema::hasTable('custom_currency_values') || \Schema::hasTable('download_extras') || \Schema::hasTable('manage_languages') || \Schema::hasTable('object_relationships') || \Schema::hasTable('options') || \Schema::hasTable('orders_items') || \Schema::hasTable('order_products') || \Schema::hasTable('posts') || \Schema::hasTable('post_extras') || \Schema::hasTable('products') || \Schema::hasTable('product_extras') || \Schema::hasTable('request_products') || \Schema::hasTable('roles') || \Schema::hasTable('role_user') || \Schema::hasTable('save_custom_designs') || \Schema::hasTable('subscriptions') || \Schema::hasTable('terms') || \Schema::hasTable('term_extras') || \Schema::hasTable('users') || \Schema::hasTable('users_custom_designs') || \Schema::hasTable('users_details') || \Schema::hasTable('user_role_permissions') || \Schema::hasTable('vendor_announcements') || \Schema::hasTable('vendor_orders') || \Schema::hasTable('vendor_packages') || \Schema::hasTable('vendor_totals')  || \Schema::hasTable('vendor_withdraws') ){
      return $is_all_tables_created = true;
    }
  }
}