...
shopist.pageLoad =
{
  ...
  {    
    ...
    if($('.send-email-to-vendor').length>0){
      $('.send-email-to-vendor').on('click', function(){
        var subject = $('#vendormailsubject').val();
        var strBody = $('#vendormailbody').val();

        if(subject == '' || strBody == ''){
          $(this).parents('#vendors_contact').find('.contact-message').remove();
          $(this).parents('#vendors_contact').find('.box-body').before('<p class="contact-message" style="padding-left:10px;color:#FF0000;">Please provide all input</p>');
        }
        ...
        if(subject != '' && strBody != ''){
          ...
          $.ajax({
            url: $('#hf_base_url').val() + '/ajax/contact-with-vendor-from-admin',
            type: 'POST',
            cache: false,
            datatype: 'json',
            headers: { 'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content') },
            data: {admin_mail:Base64.encode($(this).data('admin_email')), vendor_mail: Base64.encode($(this).data('vendor_email')), strSubject: Base64.encode(subject), message: Base64.encode(strBody)},
            
            success: function(data)
            {
              if(data && data.status == 'success'){
                $('#vendors_contact').modal('hide');
            ...
            $.ajax({
                  ...
                  headers: { 'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content') },
                   beforeSend: function() {
                        ...
                        $(".confirm").html('<span class="spinner-border spinner-border-sm mr-2" role="status"></span> Please Wait..');
                    },
                  success: function(data){
...
//user status change
    ...
    if($('.user-status-change').length>0){
      $('.user-status-change').on('click', function (e) {
        e.preventDefault();
        var msg = '';
        var statusChangeMsg = '';
        var responseMsg = '';
        ...
        if(target == 'enable'){
          msg = "Are you sure want to enable?";
          statusChangeMsg = adminLocalizationString.yes_enable_it;
          responseMsg = "Enabled Successfully";
          status = 1;
        }
        else{
          msg = "Are you sure want to disable?";
          statusChangeMsg = adminLocalizationString.yes_disable_it;
          responseMsg = "Disabled Successfully";
          status = 0;
        }
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
                  url: $('#hf_base_url').val() + '/ajax/user-status-change',
                  type: 'POST',
                  cache: false,
                  datatype: 'json',
                  data: {id:id, status:status},
    ...
    //modal open with generated token
    if($('.openTokenModal').length>0){
      $('.openTokenModal').on('click',function(e){
        e.preventDefault();
        
        $('#getTokenModal .modal-body #token_input').val('');
        $('#getTokenModal .modal-body #token_input').val($(this).data('token'));
        $('#getTokenModal').modal({ show:true });
      });
    }

    if($('.specific-vendor-for-contact').length>0){
      $('.specific-vendor-for-contact').on('click', function(e){
        e.preventDefault();
        $('#vendors_contact').find('.popup-title').html('Contact to : ' + $(this).data('email'));
        $('.send-email-to-vendor').attr('data-vendor_email', $(this).data('email'));
        $('#vendors_contact').modal({ show:true });
      });
    }
  },
...
shopist.event = 
{
  ...
  init_dropzone:function()
  {
    ...
    {
      new customDropZone( {base_url:baseUrl, url:baseUrl + "/upload/product-related-image", token:token, uploader:'upload-manufacturers-logo', paramName:'manufacturers_logo', acceptedFiles: "application/pdf", uploadMultiple:false, maxFiles:1} );
    }
        ...
        init: function() 
        {
          ...
          this.on("success", function(file, response) 
          {
            ...
            {  
              ...
              {
                $('.manufacturers-img a').attr('href', base_url + '/public/uploads/' + response.name);
